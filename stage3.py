...
SAVE_PRED_EVERY = 100
SNAPSHOT_DIR = "./weights/weights_stage3"
...
re = torch.load("./weights/weights_stage2/Stage2_Final.pth")
#re = torch.load("./saved_weights_final/Final_stage_2.pth")
log(f"Success Load",timestr)
model.load_state_dict(re)

# Load stage1 discriminator (CS-CT Discriminator)
...
model_CS_CT_D.load_state_dict(torch.load("./weights/weights_stage1/Stage1_D_Final.pth"))
#model_CS_CT_D.load_state_dict(torch.load("./saved_weights_final/Final_stage_1_D.pth"))
optimizer_CS_CT_D = optim.Adam(model_CS_CT_D.parameters(), lr=1e-4, betas=(0.9, 0.99))
optimizer_CS_CT_D.zero_grad()

# Load stage2 discriminator (FS-FT Discriminator)
...
model_FS_FT_D.load_state_dict(torch.load("./weights/weights_stage2/Stage2_D_Final.pth"))
#model_FS_FT_D.load_state_dict(torch.load("./saved_weights_final/Final_stage_2_D.pth"))
optimizer_FS_FT_D = optim.Adam(model_FS_FT_D.parameters(), lr=1e-4, betas=(0.9, 0.99))
optimizer_FS_FT_D.zero_grad()

# Initialization discriminator in CT-FT
...
bce_loss = torch.nn.MSELoss() ## For discriminators
...
## Fog Target dataloader
FT_loader = data.DataLoader(foggyzurichDataSet(tgt_root, ft_list_path,
                                            max_iters=max_iters,
                                            mean=IMG_MEAN, set=set),
                                            batch_size=2, shuffle=True, num_workers=0,
                                            pin_memory=True)
...
### Centroid (Prototype) information
...
for i_iter in tqdm(range(NUM_STEPS)):
    ...
    if i_iter % 3 == 0 : 
        ...
        try : 
            batch_ct = next(iter(CT_loader))
            img_ct, size_ct, name_ct, img_arry_ct = batch_ct
        ...
        tgt_out_maxvalue, tgt_mask = torch.max(feature_ct5, dim=1)
        for i in range(19):
            tgt_mask[(tgt_out_maxvalue < 0.9) * (tgt_mask == i)] = 255
        tgt_mask = F.interpolate(tgt_mask.unsqueeze(0).float(), size=(Ht, Wt), mode='nearest').squeeze(0).long()
        ...
        tgt_out_maxvalue, tgt_mask2 = torch.max(feature_ct5, dim=1)
        for i in range(19):
            tgt_mask2[(tgt_out_maxvalue < 0.9) * (tgt_mask2 == i)] = 255
        tgt_mask2 = F.interpolate(tgt_mask2.unsqueeze(0).float(), size=(Ht, Wt), mode='nearest').squeeze(0).long()
        ...
        #loss_adv.backward()
        loss_seg_feat_adv = loss_seg+loss_seg2+loss_feat1+loss_feat2+loss_adv+0.0001*loss_con
    ...
    if i_iter % 3 == 1 : 
        ...
        tgt_out_maxvalue, tgt_mask = torch.max(feature_ft5, dim=1)
        for i in range(19):
            tgt_mask[(tgt_out_maxvalue < 0.9) * (tgt_mask == i)] = 255
        tgt_mask = F.interpolate(tgt_mask.unsqueeze(0).float(), size=(Ht, Wt), mode='nearest').squeeze(0).long()
        tgt_mask = tgt_mask.contiguous().view(B * Ht * Wt, )
        ...
        tgt_out_maxvalue, tgt_mask2 = torch.max(feature_ft5, dim=1)
        for i in range(19):
            tgt_mask2[(tgt_out_maxvalue < 0.9) * (tgt_mask2 == i)] = 255
        tgt_mask2 = F.interpolate(tgt_mask2.unsqueeze(0).float(), size=(Ht, Wt), mode='nearest').squeeze(0).long()
        tgt_mask2 = tgt_mask2.contiguous().view(B * Ht * Wt, )
    ...
    if i_iter % 3 == 2 : 
        ...
        src_out_maxvalue, src_mask = torch.max(feature_ct5, dim=1)
        for i in range(19):
            src_mask[(src_out_maxvalue < 0.9) * (src_mask == i)] = 255
        src_mask = F.interpolate(src_mask.unsqueeze(0).float(), size=(Hs, Ws), mode='nearest').squeeze(0).long()
        src_mask = src_mask.contiguous().view(B * Hs * Ws, )
        ...
        src_out_maxvalue, src_mask2 = torch.max(feature_ct5, dim=1)
        for i in range(19):
            src_mask2[(src_out_maxvalue < 0.9) * (src_mask2 == i)] = 255
        src_mask2 = F.interpolate(src_mask2.unsqueeze(0).float(), size=(Hs, Ws), mode='nearest').squeeze(0).long()
        src_mask2 = src_mask2.contiguous().view(B * Hs * Ws, )
        ...
        loss_adv = loss_adv*0.01
        #loss_adv.backward()
        loss_seg_feat_adv = loss_feat1+loss_feat2+loss_adv
        loss_seg_feat_adv.backward()
    ...
    if i_iter % 10 == 0:
        ...
        log(f"iter = {i_iter}/{NUM_STEPS},loss_seg = {loss_seg.item()},loss_seg2 = {loss_seg2.item()}, loss_adv = {loss_adv.item()}, loss_D_s = {loss_D_source.item()}, loss_D_t = {loss_D_target.item()}", timestr)

    if i_iter >= NUM_STEPS_STOP - 1:
        torch.save(model.state_dict(), osp.join(SNAPSHOT_DIR, 'Stage3_Final.pth'))
        torch.save(model_CS_CT_D.state_dict(), osp.join(SNAPSHOT_DIR, 'Stage3_Final_CS_CT_D.pth'))
        torch.save(model_FS_FT_D.state_dict(), osp.join(SNAPSHOT_DIR, 'Stage3_Final_FS_FT_D.pth'))
        torch.save(model_CT_FT_D.state_dict(), osp.join(SNAPSHOT_DIR, 'Stage3_Final_CT_FT_D.pth'))
        break
    ...
    if i_iter % SAVE_PRED_EVERY == 0 and i_iter != 0:
        ...
        evaluate(model,log,timestr)
        torch.save(model.state_dict(), osp.join(SNAPSHOT_DIR, 'Stage3_' + str(i_iter) + '.pth'))
        torch.save(model_CS_CT_D.state_dict(), osp.join(SNAPSHOT_DIR, 'Stage3_' + str(i_iter) + 'CS_CT_D.pth'))
        torch.save(model_FS_FT_D.state_dict(), osp.join(SNAPSHOT_DIR, 'Stage3_' + str(i_iter) + 'FS_FT_D.pth'))
        torch.save(model_CT_FT_D.state_dict(), osp.join(SNAPSHOT_DIR, 'Stage3_' + str(i_iter) + 'CT_FT_D.pth'))
