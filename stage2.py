...
SAVE_PRED_EVERY = 100
SNAPSHOT_DIR = "./weights/weights_stage2"
NUM_STEPS_STOP = 20000 ## Warming up steps
...
### Load stage1 model 
re = torch.load("./weights/weights_stage1/Stage1_Final.pth")
#re = torch.load("./saved_weights_final/Final_stage_1.pth")
log(f"Success Load",timestr)
model.load_state_dict(re)

# Load stage1 discriminator 
...
model_D.load_state_dict(torch.load("./weights/weights_stage1/Stage1_D_Final.pth"))
#model_D.load_state_dict(torch.load("./saved_weights_final/Final_stage_1_D.pth"))
...
for i_iter in tqdm(range(NUM_STEPS)):
    ...
    if i_iter >= NUM_STEPS_STOP - 1:
        torch.save(model.state_dict(), osp.join(SNAPSHOT_DIR, 'Stage2_Final.pth'))
        torch.save(model_D.state_dict(), osp.join(SNAPSHOT_DIR, 'Stage2_D_Final.pth'))
        break

    if i_iter % SAVE_PRED_EVERY == 0 and i_iter != 0:
        print('taking snapshot ...')
        evaluate(model,log,timestr)
        torch.save(model.state_dict(), osp.join(SNAPSHOT_DIR, 'Stage2_' + str(i_iter) + '.pth'))
        torch.save(model_D.state_dict(), osp.join(SNAPSHOT_DIR, 'Stage2_' + str(i_iter) + '_D.pth'))
