...
class foggyzurichDataSet(data.Dataset):
    ...
    def __init__(self, root, list_path,label_path, max_iters=None, mean=(128, 128, 128), ignore_label=255, set='val'):
        self.root = root
        ...
        for name in self.img_ids:
            ...
            label_file = self.label_path + '/' + name.split('/')[-1]
            self.files.append({
                "img": img_file,
                "label" : label_file,
                "name": name
    ...
    def __getitem__(self, index):
        ...
        upper = random.randint(0, image_height - crop_height)
    ...
    def _apply_transform(self, img,lbl, scale=(0.7, 1.3), crop_size=600):
        (W, H) = img.size[:2]
